...
Revision History
    ...
    2022-01-13 - Cleanup
    2026-08-01 - Simplified report export flow and reduced repeated object materialization
...
# Create CSV Report
$reportSets = @(
    [pscustomobject]@{ Path = $logPath; Data = $azureADPWSummary },
    [pscustomobject]@{ Path = $logPath1; Data = $azureADAgentSummary },
    [pscustomobject]@{ Path = $logPath2; Data = $azureADProxySummary }
)

foreach ($reportSet in $reportSets) {
    $reportSet.Data | Export-Csv -Path $reportSet.Path -NoTypeInformation -Encoding UTF8
}

# Create HTML Report
$azureADPWSummaryR = if ($azureADPWSummary.Count -gt 0) {
    $azureADPWSummary | ConvertTo-Html -Head $style
}
else {
    '<p>No Azure AD password protection summary data was returned.</p>'
}
$azureADAgentSummaryR = if ($azureADAgentSummary.Count -gt 0) {
    $azureADAgentSummary | ConvertTo-Html
}
else {
    '<p>No Azure AD password protection agent data was returned.</p>'
}
$azureADProxySummaryR = if ($azureADProxySummary.Count -gt 0) {
    $azureADProxySummary | ConvertTo-Html
}
else {
    '<p>No Azure AD password protection proxy data was returned.</p>'
}
